...
# Extra for attention-blocks
def BNReLU(in_channels, bn_type=None):
...
class _SelfAttentionBlock(nn.Module):
    ...
    def __init__(self, in_channels, key_channels, value_channels, out_channels=None, scale=1, bn_type=None):
        ...
        self.W = nn.Sequential(
            nn.Conv2d(in_channels=self.value_channels, out_channels=self.out_channels, kernel_size=1, stride=1, padding=0),
            BNReLU(self.out_channels, bn_type=bn_type),
        )
    ...
    def __init__(self, in_channels, key_channels, value_channels, out_channels, down_factors=[[8,8]], dropout=0, bn_type=None):
        ...
        if len(self.down_factors) > 1:
            self.up_conv = nn.Sequential(
                ...
                BNReLU(len(self.down_factors) * out_channels, bn_type=bn_type),
            )
...
if __name__ == "__main__":
    ...
    baseoc_infer = LongShortSelfAttentionModule(in_channels=512,
                                 key_channels=256,
                                 value_channels=512,
                                 out_channels=512,
                                 dropout=0,
